...
const loginAdmin = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[loginAdmin] Admin ${admin.id} logged in successfully, cookie set. Token expires in 8h. Cookie secure: ${process.env.NODE_ENV === "production"}, sameSite: Lax.`,
    );
    res.json({ admin });
  } catch (error) {
...
const getAdminStatus = async (req, res) => {
  console.log(
    `[getAdminStatus] Entering controller. req.admin is:`,
    req.admin ? "Defined" : "Undefined",
  );
  try {
    const admin = req.admin;
    if (!admin) {
      console.log(
        "[getAdminStatus] No admin object on request, returning unauthenticated.",
      );
      return res.status(200).json({ isLoggedIn: false, admin: null });
    }
    console.log(
      `[getAdminStatus] Admin ${admin.id} found on request, returning authenticated.`,
    );
